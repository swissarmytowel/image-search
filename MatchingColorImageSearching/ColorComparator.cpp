...
//Pixel data
	...
	double deltaH = sqrt ( ( referenceA - sampleA )*( referenceA - sampleA ) +
		( referenceB - sampleB )*( referenceB - sampleB ) - deltaC*deltaC );

		//Compensations calculation
	double lightnessCompensation = referenceL < 16.0 ? 0.511 :
		( 0.040975 * referenceL ) / ( 1.0 + 0.01765 * referenceL );
	...
	double t = hue >= 164.0  &&  hue <= 345.0 ?
		0.56 + abs ( 0.2*cos ( ( hue + 168.0 ) * ( pi / 180.0 ) ) ) :
		0.36 + ( abs ( 0.4*cos ( ( hue + 35.0 ) * ( pi / 180.0 ) ) ) );

	double hueCompensation = chromaCompensation * ( f * t + 1.0 - f );

	_deltaE = calculateMagnitude ( deltaL / ( 2.0 * lightnessCompensation ),
								   deltaC / chromaCompensation, deltaH / hueCompensation );
